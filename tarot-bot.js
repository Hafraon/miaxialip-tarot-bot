...
async function handleHoroscope(chatId) {
    try {
        ...
        const horoscope = `⭐ **ЗОРЯНИЙ ПРОГНОЗ СПЕЦІАЛЬНО ДЛЯ ТЕБЕ**

${sign.emoji} **Привіт, прекрасний ${sign.name}!** (${sign.period})

🌟 Сьогодні зірки шепочуть твоє ім'я... Вони обрали для тебе особливу карту:

🔮 **Твоя карта-покровителька:** ${card.emoji} ${card.name}

✨ **Що кажуть зірки:** ${card.meaning}

${sign.element === 'Вогонь' ? '🔥 **Вогняна душе!** Сьогодні твоя енергія палає яскравіше зірок! Використай цю силу для втілення найсміливіших мрій.' :
  sign.element === 'Земля' ? '🌱 **Земна мудрице!** Сьогодні - твій день для створення міцного фундаменту. Довіряй своїй практичності та інтуїції.' :
  sign.element === 'Повітря' ? '🌬️ **Легка як вітер!** Сьогодні твої слова мають особливу силу. Говори від серця, і світ почує тебе.' :
  '🌊 **Глибока як океан!** Сьогодні твоя інтуїція говорить голосніше за логіку. Слухай своє серце - воно знає правильний шлях.'}

💫 *Це лише натяк від Всесвіту. Для глибшого розуміння свого шляху давай поговоримо особисто!*
...
async function handleFreeReading(chatId) {
    try {
        ...
        const reading = `🔮 **ТВІЙ ОСОБИСТИЙ РОЗКЛАД НА ДЕНЬ**

✨ Карти обрали тебе не випадково! Давай разом подивимося, що приготував цей день...

🌅 **Ранкова енергія:** ${cards[0].emoji} ${cards[0].name}
Ранок шепоче тобі: "${cards[0].meaning}". Почни день з цією думкою у серці.

☀️ **Денна сила:** ${cards[1].emoji} ${cards[1].name}
Коли сонце буде у зеніті, пам'ятай: "${cards[1].meaning}". Це твій ключ до успіху сьогодні.

🌙 **Вечірня мудрість:** ${cards[2].emoji} ${cards[2].name}
Перед сном поміркуй над цим: "${cards[2].meaning}". Нехай ця думка супроводжує тебе у сни.

💫 *Довіряй картам, довіряй собі. Кожен день - це нова можливість стати кращою версією себе!*
...
async function handleLoveReading(chatId) {
    try {
        ...
        const reading = `💝 **ТАЄМНИЦІ ТВОГО СЕРЦЯ**

💖 Закрий очі, відчуй як б'ється твоє серце... Карти кохання готові розкрити свої секрети...

💕 **Твоє серце сьогодні:** ${cards[0].emoji} ${cards[0].name}
Відчуваєш це? "${cards[0].meaning}" - саме це намагається сказати тобі твоя душа про кохання.

🌹 **Що шепоче доля:** ${cards[1].emoji} ${cards[1].name}
А ось що чекає на тебе у світі почуттів: "${cards[1].meaning}". Довіряй цим знакам.

✨ *Пам'ятай, кохана: найкрасивіші стосунки народжуються тоді, коли ти по-справжньому любиш себе. Ти варта найщирішого кохання!*

💖 **Хочеш дізнатися більше про свою любовну долю?** Детальний розклад тільки 280 грн!`;
...
async function handleUserQuestion(chatId, question) {
    try {
        ...
        const response = `🔮 **ВСЕСВІТ ВІДПОВІДАЄ ТОБІ**

❓ **Твоє питання звучало так:** "${question}"

✨ І знаєш що? Карти почули тебе! Вони вже тримають відповідь...

🃏 **Твоя карта-порадниця:** ${card.emoji} ${card.name}

💫 **Ось що вона шепоче:** "${card.meaning}"

🌟 **Від серця до серця:** Ця карта не випадково прийшла до тебе. Вона підказує: довіряй своєму внутрішньому голосу. Відповідь, яку ти шукаєш, вже живе у твоєму серці. Іноді нам просто потрібен ніжний поштовх, щоб її почути.

💝 **Відчуваєш, що потрібно більше ясності?** Детальна консультація всього 70 грн!`;
...
async function handleQuestionPrompt(chatId) {
    try {
        const promptMessage = `🎯 **РОЗКАЖИ МЕНІ, ЩО НА СЕРЦІ**

💫 Знаєш, іноді найважче - це сформулювати те, що насправді хвилює. Але ти вже тут, а це означає, що готова шукати відповіді.

🌟 **Я тут, щоб тебе вислухати.** Розкажи мені про те, що турбує твою душу, і карти дадуть тобі підказку...

💡 **Ось кілька ідей, якщо важко почати:**
• "Мені здається, варто змінити роботу... Чи правильно я відчуваю?"
• "Як мені зробити стосунки щасливішими?"
• "Що приготував для мене цей місяць?"
• "Чи іду я правильним шляхом у житті?"
• "Чи варто мені довіряти цій людині?"

✨ **Просто пиши від серця** - наступним повідомленням розкажи мені, що тебе хвилює. Я обов'язково почую і допоможу знайти відповідь! 💝`;
